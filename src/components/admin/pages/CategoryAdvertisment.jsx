    ...
    const handleDelete = async (id) => {
        try {
            ...
            if (response.ok) {
                ...
                // Full window refresh for UI update as per original code
    ...
    return (
        <div className="w-full max-w-[1400px] mx-auto space-y-8 p-6 font-sans pb-24">
            ...
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-2">
                <div>
                    <h1 className="text-2xl font-semibold tracking-tight text-slate-900">Category Advertisement</h1>
                    <p className="text-sm text-slate-500 mt-1">Manage banner images and links for specific categories.</p>
                </div>
                {editBanner && (
                    <Button type="button" variant="outline" onClick={() => {
                        setEditBanner(null);
                        setFormData({ buttonLink: "", image: { url: "", key: "" } });
                        ...
                        <CardHeader className="border-b border-slate-50 bg-white/50 pb-6">
                            <div className="flex items-center gap-2">
                                ...
                                <CardTitle className="text-lg font-semibold text-slate-800">
                                    {editBanner ? "Edit Category Banner" : "Add New Category Banner"}
                                </CardTitle>
                            </div>
                            <CardDescription className="text-slate-500">Upload a banner image and configure its link.</CardDescription>
                        </CardHeader>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-8">
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                    ...
                                    <div className="space-y-3">
                                        <Label className="text-sm font-medium text-slate-600 ml-1">Category Banner Image <span className="text-red-500">*</span></Label>
                                        ...
                                        {!formData.image.url ? (
                                            <div 
                                                ...
                                                className={`border-2 border-dashed rounded-2xl h-[180px] flex flex-col items-center justify-center gap-3 cursor-pointer transition-colors
                                                ...
                                                {uploading ? (
                                                    ...
                                                    <>
                                                        ...
                                                        <div className="text-center px-4">
                                                            ...
                                                            <p className="text-xs text-slate-500 mt-1">Recommended: Category dimensions</p>
                                                        </div>
                                                    </>
                                                )}
                                            </div>
                                        ) : (
                                            <div className="relative w-full h-[180px] border border-slate-100 rounded-2xl overflow-hidden bg-slate-50 group">
                                                <Image
                                    ...
                                    <div className="space-y-5 flex flex-col justify-center">
                                        <div className="grid gap-2">
                                            <Label className="text-sm font-medium text-slate-600 ml-1">Target URL <span className="text-red-500">*</span></Label>
                                            ...
                                            <p className="text-xs text-slate-500 ml-1 mt-1">Users will be redirected to this link when they click the banner.</p>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <Card className="rounded-[20px] border-slate-100 shadow-sm bg-white overflow-hidden h-full">
                        <CardHeader className="border-b border-slate-50 bg-white/50 pb-6">
                            <div className="flex items-center gap-2">
                                ...
                                <CardTitle className="text-lg font-semibold text-slate-800">Existing Category Banners</CardTitle>
                            </div>
                            <CardDescription className="text-slate-500 mt-1">Review and manage your active category advertisements.</CardDescription>
                        </CardHeader>
                                            ...
                                            banners.map((banner, index) => (
                                                <TableRow key={banner._id} className="border-b border-slate-50 hover:bg-slate-50/80 transition-colors group">
                                                    ...
                                                    <TableCell className="py-4 text-center">
                                                        <div className="relative w-24 h-12 mx-auto rounded-lg overflow-hidden border border-slate-100 bg-slate-50">
                                                            {banner.image?.url ? (
                                                                <Image src={banner.image.url} alt="Category Banner" fill className="object-cover" />
                                                            ) : (
            ...
            <Dialog open={showDeleteModal} onOpenChange={setShowDeleteModal}>
                <DialogContent className="rounded-[24px] p-6 border-slate-100 shadow-xl bg-white max-w-md font-sans gap-0">
                    <DialogHeader className="mb-4">
                        <DialogTitle className="text-xl font-semibold text-slate-800">Delete Advertisement</DialogTitle>
                    </DialogHeader>
                    <p className="text-slate-600 mb-6">Are you sure you want to delete this category advertisement? This action cannot be undone.</p>
